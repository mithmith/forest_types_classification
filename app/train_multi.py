...
from app.loss import calculate_iou, iou_loss, multi_class_iou_loss, calculate_multiclass_iou
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for i, (sample, mask) in enumerate(
            ...
            batch_masks.append(torch.tensor(mask.copy(), dtype=torch.float32))
            ...
            if len(batch_inputs) == batch_size or (i + 1) == len(train_dataset):
                ...
                # Calculate IoU
                iou = calculate_multiclass_iou(F.softmax(outputs, dim=1), mask_batch)
                total_iou += iou
            ...
            if (i + 1) % 100 == 0:
                ...
                # Подготовка данных
                if batch_size > 1:
                    model_out_mask = F.softmax(outputs, dim=1).detach().cpu().squeeze().numpy()[0]  # Предсказанная маска
                    ...
                    model_out_mask = F.softmax(outputs, dim=1).detach().cpu().squeeze().numpy()
                ...
                if len(predicted_mask.shape) == 3:
                    predicted_mask = np.max(predicted_mask[1:], axis=0)
                    ground_truth_mask = np.max(ground_truth_mask[1:], axis=0)
...
def evaluate(model, x1: np.ndarray) -> np.ndarray:
    ...
    with torch.no_grad():
        ...
        return F.softmax(outputs, dim=1).detach().cpu().squeeze().numpy()

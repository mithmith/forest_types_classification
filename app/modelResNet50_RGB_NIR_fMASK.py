...
class ResNet50_RGB_NIR_fMASK_Model(nn.Module):
    ...
    def train_model(self, train_dataset, val_dataset, epochs=1, batch_size=1, learning_rate=0.001, device="cuda"):
        ...
        for epoch in range(epochs):
            ...
            for i, (sample, mask) in enumerate(
                train_dataset.get_next_generated_sample(verbose=False, exclude_nir=False, exclude_fMASK=False)
            ):
    ...
    def validate(self, val_dataset, criterion, batch_size: int, device: str):
        ...
        with torch.no_grad():
            for i, (sample, mask) in enumerate(
                val_dataset.get_next_generated_sample(verbose=False, exclude_nir=False, exclude_fMASK=False)
            ):

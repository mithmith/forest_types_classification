...
class ForestTypesDataset:
    ...
    def find_geojson_paths_for_img(self, img: str):
        class_names = ["deforestation", "burns", "drying", "windfalls"]
        first_part = img.split("_")[2]
        second_part = img.split("_")[5]
        masks_paths = {}
        for class_name in class_names:
            for s in self.geojson_files:
                # We assume the geojson filename contains the first and second parts plus the class name.
                if first_part in str(s) and second_part in str(s) and class_name in str(s).lower():
